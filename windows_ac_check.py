"""
windows_ac_check.py (ENHANCED)

Windows access-control checks via PowerShell (best-effort).
Enhancements focus on checking built-in account status and password hash storage policy.
...
Usage:
    ...
    admin_status = wac.check_built_in_admin_status() # NEW
"""
...
# --- ENHANCEMENT 1: Check Built-in Administrator Status ---
def check_built_in_admin_status() -> Dict[str, Any]:
    """
    Checks the status (Enabled/Disabled) and Name of the built-in Administrator account (SID S-1-5-500).
    Best practice is to rename it and disable it or keep it disabled.
    """
    result = {"IsRenamed": False, "IsEnabled": None, "CurrentName": None}

    # Find the built-in Administrator account by its well-known SID (S-1-5-500)
    try:
        ps_cmd = r"""
        try {
            # Find the user object by SID
            $adminUser = Get-LocalUser -SID "S-1-5-500" -ErrorAction Stop
            
            # Check if the name is still 'Administrator'
            $result = @{
                CurrentName = $adminUser.Name
                IsEnabled = $adminUser.Enabled
                IsRenamed = ($adminUser.Name -ne "Administrator")
            }
        ...
        result["ComplianceNote"] = "Should be disabled or renamed."
...
def get_local_users() -> List[Dict[str, Any]]:
    ...
    Returns list of local users with fields:
      Name, Enabled (bool), LastLogon (datetime or empty), PasswordExpired, PasswordRequired, FullName, Description
    """
    ps_cmd = (
...
def get_password_policy() -> Dict[str, Optional[Any]]:
    """
    Attempts to extract local password/account policy using `net accounts` and enhanced policy checks.
    """
    ...
    policy = {
        ...
        "StorePasswordUsingReversibleEncryption": None, # NEW
        "MinimumPasswordAgeDays": None, # NEW
    }
    ...
    # Try to check local security policy (PasswordComplexity & Hashing) via secedit export (enhanced)
    try:
        ...
        for line in cfg.splitlines():
            ...
            if line.startswith("PasswordComplexity"):
                _, val = line.split("=", 1)
                # 1 = Enabled (secure), 0 = Disabled
                policy["PasswordComplexity"] = bool(int(val.strip()))
            
            # --- ENHANCEMENT: Reversible Encryption Check ---
            elif line.startswith("ClearTextPassword"):
                _, val = line.split("=", 1)
                # 0 = Disabled (secure), 1 = Enabled
                policy["StorePasswordUsingReversibleEncryption"] = bool(int(val.strip()))
                
            if line.startswith("MinimumPasswordLength"):
                try:
                    # Overwrite net accounts value with more reliable secedit value
                    policy["MinimumPasswordLength"] = int(line.split("=", 1)[1].strip())
                except:
                    pass
    except Exception as e:
        policy["SeceditError"] = str(e)
...
def check_windows_hello() -> Dict[str, Any]:
    ...
    try:
        ...
        if result["NgcExists"]:
            try:
                # Attempt to get contents, which fails if permissions are tight (as expected)
...
if __name__ == "__main__":
    ...
    try:
        admin_status = check_built_in_admin_status()
